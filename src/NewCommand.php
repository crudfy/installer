...
namespace Crudfy\Installer\Console;
...
class NewCommand extends Command
{
    ...
    protected function configure()
    {
        $this
            ->setName('new')
            ->setDescription('Create a new Crudfy application')
...
//        }
        if (!is_dir($directory)) {
            $process = Process::fromShellCommandline("mkdir $name");
    ...
    private function download($directory) {
        $fullpath = $directory.'/project.zip';
        $client = new Client();
        $res = $client->request('GET', 'http://192.168.43.103:8282/projects', ['sink' => $fullpath]);
        $zip = new ZipArchive;
        if ($zip->open($fullpath)) {
            $zip->extractTo($directory);
            $zip->close();
        }
        $this->moveDirectory($directory.'/project', $directory);
        unlink($fullpath);
    }

    public function moveDirectory($sourceDirectory, $destinationDirectory) {
        $files = scandir($sourceDirectory);
        foreach ($files as $file) {
            if (!in_array($file, ['.', '..'])) {
                rename($sourceDirectory.'/'.$file, $destinationDirectory.'/'.$file);
            }
        }
